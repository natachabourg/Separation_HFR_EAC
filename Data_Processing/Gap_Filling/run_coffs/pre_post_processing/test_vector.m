...
addpath('/data/MIO/natachab/dineof_hourly_2020_2021/functions/');
addpath('/home/natachab/Bureau/eac_chloro_hfr_analysis/hfr_data_processing/qc/merged_qc/');

path = '/home/natachab/Bureau/eac_chloro_hfr_analysis/hfr_data_processing/gap_filling/run_coffs/';
path_result = [path 'my_result_folder/log5/'];

ini_file = {[path 'pre_post_processing/hfr_hourly_gridded_fromfile_RRK_Y2019_M7.nc'],...
    [path 'pre_post_processing/hfr_hourly_gridded_fromfile_NNB_Y2019_M7.nc']};

data_file = {[path 'my_result_folder/log5/daily_2019_2021_RRK_din.nc'],...
    [path 'my_result_folder/log5/daily_2019_2021_NNB_din.nc']};

time_file = {[path 'pre_post_processing/daily_2019_2021_RRK_pre.nc'],...
    [path 'pre_post_processing/daily_2019_2021_NNB_pre.nc']};
...
for i =1:2
    ...
    DATEjulian = ncread(time_file{i},'time');
...
%%
ncfile = [path_result 'currents_dineof_COFFS_Y2019M07_Y2021M12.nc'];
...
for i = 1:length(xr)
    for j = 1:length(yr)
        [dlod(i,j),dlon(i,j)]=min(abs(xn-xr(i)));
        [dlad(i,j),dlat(i,j)]=min(abs(yn-yr(j)));
    end
end

%% Combine radials to vectors
    ...
    for j = 1:length(yr)
        if (~isnan(dlon(i,j)))
            for k = 1:length(tr)
                ...
                if ~isempty(ind) && ~isnan(rr(i,j,k)) && ~isnan(rn(dlon(i,j),dlat(i,j),ind))
                    %count2(i,j)  = count2(i,j) + 1;

                    % North Nambucca
                    RN = rn(dlon(i,j),dlat(i,j),ind); 
                    DN = dn(dlon(i,j),dlat(i,j),ind);
                    DN(RN<0) = DN(RN<0)-180;
                    DN(DN<0) = DN(DN<0) + 360;
                    ...
                    % Combine radials to vectors
                    rad_Speed = [abs(RR) abs(RN)];
                    angle = [DR DN];
                    [v(i,j,k),u(i,j,k)] = tuvLS(rad_Speed,angle);
                end
...
t = tr;lon = xr; lat = yr; LON = Xr; LAT = Yr;
...
ang1(ave_1<0) = ang1(ave_1<0)-180;
ang1(ang1<0) = ang1(ang1<0) + 360;


ang2(ave_2<0) = ang2(ave_2<0)-180;
ang2(ang2<0) = ang2(ang2<0) + 360;

for i = 1:size(lon_rhed,1)
    for j = 1:size(lon_rhed,2)
        
        dlon_pixel = abs(lon_rhed(i,j) - lon_seal);
        dlat_pixel = abs(lat_rhed(i,j) - lat_seal);
        
        distance_pixel = dlon_pixel+dlat_pixel;
        
        ind = distance_pixel==nanmin(nanmin(distance_pixel));
        
        
        
        rad_Speed = [ave_1(i,j) ave_2(ind)];
        angle = [ang1(i,j) ang2(ind)];
        
        [u(i,j), v(i,j)] = tuvLS(rad_Speed, angle);
...
pcolor(lon_rhed, lat_rhed, ave_1); shading flat
colorbar
colormap(jet)
...
pcolor(lon_seal, lat_seal, ave_2); shading flat
colorbar
colormap(jet)


figure;
quiver(lon_rhed,lat_rhed,u,v)
