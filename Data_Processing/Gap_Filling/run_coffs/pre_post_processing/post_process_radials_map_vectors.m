...
data_file = {[path_result 'daily_2019_2021_RRK_din.nc'],...
    [path_result 'daily_2019_2021_NNB_din.nc']};

pre_file = [path 'pre_post_processing/daily_2019_2021_RRK_pre.nc'];


vr1 = ncread(data_file{1},'v');
ang1 = ncread(ini_file{1},'vr_dir');
ang1 = nanmean(ang1,3);
lon_rrk = ncread(ini_file{1},'lon');
lat_rrk = ncread(ini_file{1},'lat');
lon0_rrk = 153.23;
lat0_rrk = -29.98;
[xr1, yr1] = lonlat2km(lon0_rrk, lat0_rrk, lon_rrk, lat_rrk);

vr2 = ncread(data_file{2}, 'v');
ang2 = ncread(ini_file{2}, 'vr_dir');
ang2 = nanmean(ang2, 3);

lon_nnb = ncread(ini_file{2},'lon');
lat_nnb = ncread(ini_file{2},'lat');
lon0_nnb = 153.01;
lat0_nnb = -30.62;
[xr2, yr2] = lonlat2km(lon0_nnb, lat0_nnb, lon_nnb, lat_nnb);

lon0 = {lon0_nnb, lon0_nnb};
lat0 = {lat0_nnb, lat0_nnb};
STA  = {'RRK', 'NNB'};

% Post process the data (Remove bad points, see notebook for explanations)

mask_rrk = readNPY('PostProcessCoffsHarbourMasks_maskrrk.npy')
mask_nnb = readNPY('PostProcessCoffsHarbourMasks_masknnb.npy')

mask_all_time = readNPY('PostProcessCoffsHarbourMasks_maskall_time.npy')

vr1_masked = vr1.*mask_rrk';
vr2_masked = vr2.*mask_nnb';
...
%%
ncfile = [path_result 'currents_dineof_COFFS_Y2019M07_Y2020M12.nc'];
