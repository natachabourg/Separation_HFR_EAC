...
%
% Modif: BOURG 2020/05/15: change the end of month from 23.59 to 23.01 
%
%                          prevents the program from stopping when 0 data
%                          for a month

function nch2nc(radial_data_params,RADAR_infos)

%%% cll or xyv path
%%% This directory contains files sorted either in decadal directories or not
PATH_ORI = radial_data_params.path_input;

%%% NetCDF path
PATH_NC = radial_data_params.path_output;
...
if ~isempty(radial_data_params.dates)  % range date case
    ...
    dates(2,:)=datenum(NY,NM,NDmax,23,01,0)-datenum(2010,1,1,0,0,0); 
...
[ cxname ] = nc_list( PATH_ORI , EXT , STA );
...
if ~exist('i1','var')
    disp([STA ': The first date of "radial_data_params" might be posterior to the most recent existing file!']);
    return; 
end
if ~exist('i2','var')
    disp([STA ': The last date of "radial_data_params" might be anterior to the oldest existing file!']);
    return; %
end

cxname = cxname(i1:i2,:);
%%% read radial velocities, x/y grid and date
[ vr, lonr,latr,angr, DATE ] = nc_reader( cxname );
%  DATE.calendar
%  pause
%data=nc_reader ( cxname );
%AJM data = import_ncdata(radial_data_params(active_sites(i_radar)),RADAR_infos(active_sites(i_radar)),[map.lon0 map.lat0]);
...
%    FILL_miss_time_NAN_withoutBorEAJM( vr , DATE.calendar, DATE.julian , DATESrange.julian );
...
nccreate(ncfile,'v',...
    ...
    'Format','classic')
nccreate(ncfile,'spatial_coverage',...
    'Dimensions',{'time',length(DATEjulian)}, ...
    'Format','classic'); %NB
...
ncwriteatt(ncfile, 'spatial_coverage', 'comment', char('% of pixels that are not NaN'));
...
v = ncread(ncfile,'v');
[x,y,timee] = size(v);
spat_cov = zeros(timee,1);

for t = 1 : timee
    velo = v(:,:,t);
    [x1, y1] = size(velo);
    nb_pixel = x1*y1;
    velo_nan = velo(~isnan(velo));
    nb_nan = size(velo_nan);
    ratio = 100*nb_nan(1)/nb_pixel;
    spat_cov(t) = ratio;
end


ncwrite(ncfile,'spatial_coverage',spat_cov); %NB
