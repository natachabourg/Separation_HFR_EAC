function [ VAR_EXT, DATE_EXT , TIME_NONAN ] = ...
    FILL_miss_time_NAN_withoutBorEAJM( VAR , datecal, datejulian, DATESjulian ,accur, time_origin )
...
if ndims(VAR) == 3
    ...
    TIME_NONAN=zeros(NB_TIME,1);
    TIME_NONAN(1)=1;
for i=2:length(datecal(:,9:10))
    Daa(i) = str2double(datecal(i,9:10));
    Haa(i) = str2double(datecal(i,12:13));
    if (Daa(i)-Daa(i-1)) > 0 %next day
        delta(i)= abs(Haa(i)-Haa(i-1)+24);
    else
        delta(i)= abs(Haa(i)-Haa(i-1));
    end
    if delta(i) == 0  
        double_time=1
        pause
...
for i=2:NB_TIME %length(DATE.calendar)
    datejulian_EXT(i)=datejulian_EXT(i-1) + time_step;   
...
%         anne=i
  %  for i=2:NB_TIME
   %     datejulian_EXT(i)=datejulian_EXT(i-1) + time_step;
      if abs(datejulian_EXT(i)-datejulian(k))  < accur
            datejulian_EXT(i)=datejulian(k);
            VAR_EXT(i,:)=squeeze(VAR(k,:)); %%%%DIM=3 %VAR_EXT(i,:,:)=squeeze(VAR(k,:,:));
            TIME_NONAN(i)=1;
            k=k+1;
      else
          i
