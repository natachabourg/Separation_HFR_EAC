...
function plot_save_current(map_opts,currents,cartesian_grid,params)
...
if isfield(cartesian_grid,'coverage')
    ...
    if map_opts.lonlat_xy == 1
        h = m_pcolor(cartesian_grid.lon,cartesian_grid.lat,coverage);
...
if map_opts.lonlat_xy == 1
    ...
    m_quiver(lon_legend,lat_legend,scale,0,'Color','r','MaxHeadSize',2,'LineWidth',2);
    m_text(lon_legend,lat_legend+cartesian_grid.step/100,'1 m/s','Color','r','FontSize',10);
else
...
for i_time = length(currents)-20 : length(currents)      % 1 : length(currents)
    ...
    if map_opts.lonlat_xy == 1
        h_pcolor = m_pcolor(cartesian_grid.lon,cartesian_grid.lat,sqrt(u.^2 + v.^2)/scale);
        set(h_pcolor,'LineStyle','none');
        caxis([0 0.7]);
        h_quiver = m_quiver(cartesian_grid.lon,cartesian_grid.lat,u,v,0,'k','LineWidth',0.5,'MarkerSize',1);
    else
        h_quiver =   quiver(cartesian_grid.x,  cartesian_grid.y,  u,v,0,'b','LineWidth',0.5,'MarkerSize',1);
    end
    title(date.calendar);
    
    % Save the figure, if asked
    if params.save == 1
        if ~exist([params.path filesep date.radar '.png'],'file')
