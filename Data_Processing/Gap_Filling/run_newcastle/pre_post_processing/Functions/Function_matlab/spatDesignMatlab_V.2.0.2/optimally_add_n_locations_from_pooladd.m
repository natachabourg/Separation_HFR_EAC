function [xx,yy,avgkrigemsep]=optimally_add_n_locations_from_pooladd(externaldrift,U,x0,y0,pooladdxy,sigma2,apriorivar,w,M,delta,n,xlower,xupper,ylower,yupper,maxiterations,polygon,nx,ny,criterion)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% [xx,yy,avgkrigemsep]=optimally_add_n_locations_from_pooladd(externaldrift,U,x0,y0,pooladdxy,sigma2,apriorivar,w,M,delta,n,xlower,xupper,ylower,yupper,maxiterations,polygon,nx,ny,criterion)
%
...
%                 x-coordinates of the designpoints for an i*2-point
%                 design. 
%
%  yy.............a cell array of size n. cell{i} gives the optimal
%                 y-coordinates of the designpoints for an i*2-point
%                 design.
%
...
for i=1:ceil(n/2)
 ...
 x1=[x0;x];
 y1=[y0;y];
 [xstarting,ystarting,avgmsep,infM_1]=add_n_locations_from_pooladd(externaldrift,U,x1,y1,[pooladdx,pooladdy],sigma2,apriorivar,w,M,delta,xlower,xupper,ylower,yupper,polygon,nx,ny,2,calc,infM_1,criterion);
 pooldeletex=[x;xstarting{2}'];
 pooldeletey=[y;ystarting{2}'];
 ind=[];
 for j=1:length(pooladdx)
     a=find(sqrt((xstarting{2}-pooladdx(j)).^2+(ystarting{2}-pooladdy(j)).^2)<=epsilon);
     if isempty(a)
         ind=[ind,j];
     end
 end
 pooladdx=pooladdx(ind);
 pooladdy=pooladdy(ind);
 
 [x,y,optimality1,infM_1,pooladdx,pooladdy]=optimally_improve_pooldelete_from_pooladd(externaldrift,U,x0,y0,[pooladdx,pooladdy],[pooldeletex,pooldeletey],sigma2,apriorivar,w,M,delta,xlower,xupper,ylower,yupper,maxiterations,polygon,nx,ny,0,infM_1,criterion);
 xx{i}=x;
 yy{i}=y;
