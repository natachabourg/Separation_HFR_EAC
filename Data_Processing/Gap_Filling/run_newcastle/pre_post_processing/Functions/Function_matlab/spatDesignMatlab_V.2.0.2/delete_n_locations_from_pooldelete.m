function [xx,yy,avgkrigemsep,infM_1,delx,dely]=delete_n_locations_from_pooldelete(externaldrift,U,x0,y0,pooldeletexy,sigma2,apriorivar,w,M,delta,xlower,xupper,ylower,yupper,polygon,nx,ny,n,calc,infM_1,criterion);
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  [xx,yy,avgkrigemsep,infM_1,delx,dely]=delete_n_locations_from_pooldelete(externaldrift,U,x0,y0,pooldeletexy,sigma2,apriorivar,w,M,delta,xlower,xupper,ylower,yupper,polygon,nx,ny,n,calc,infM_1,criterion);
%
%  delets n design points from an existing design.
%
%  Inputs:
%
...
%
%  x0.............the x-coordinates of the already available 
%                 design points
%
%  y0.............the y-coordinates of the already available 
%                 design points
%
...
%
%  calc...........if calc=1, the information matrix is calculated
%
...
%  Outputs:
%
%  xx............ a vector giving the optimal x-coordinates of the 
%                 new design points . 
%
%  yy.............a vector giving the optimal y-coordinates of the
%                 new design points.
%
...
for s=1:n
   ...
   for i=1:length(pooldeletex)
      h=i
      zz(i)=optimxiyidelete(pooldeletex(i),pooldeletey(i),w,M,U,infM_1,n1-s,polygon,criterion,externaldrift);
   end
   ...
   m1=n1-(s-1);
   infM_1rf=infM_1*rf;
   infM_1=((m1-1)/m1)*(infM_1+(infM_1rf*infM_1rf')/(m1-infM_1rf'*rf));
   avgkrigemsep(s)=sigma2*(1+trace(U*infM_1)/(n1-s));
   ind=[];
   for j=1:length(x0)
       a=find(sqrt((delx-x0(j)).^2 +(dely-y0(j)).^2)<=epsilon);
       if isempty(a)
           ind=[j,ind];
       end
   end
