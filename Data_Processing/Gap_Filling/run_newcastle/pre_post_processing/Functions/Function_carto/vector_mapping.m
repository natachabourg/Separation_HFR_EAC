...
%name1 = [radial_data(1).name radial_data(2).name];
%name2 = [radial_data(2).name radial_data(1).name];
name1 = 'PEY';
name2 = 'POB';
...
%% Check who's the master RADAR and arrange the data accordingly
if ~strcmp(params.master_RADAR,cartesian_grid.master_RADAR) %#ok<NODEF>
    cartesian_grid.bistatic_distance = cartesian_grid.bistatic_distance([2 1],:,:);
    cartesian_grid.bistatic_angle    = cartesian_grid.bistatic_angle([2 1],:,:);
    cartesian_grid.radial_direction  = cartesian_grid.radial_direction([2 1],:,:);
end
...
% h = waitbar(0,'Computing current velocities...');
for i_time = 1 : N_times

    % Build a structure for the present time only
    for i_site = 1 : N_sites
        data_time(i_site).lonr = radial_data(i_site).lonr;  %#ok<*AGROW>
    ...
    if all( abs(date_nearest - date) > time_acc )  % unavailable date
        ...
        switch params.mapping.method
            ...
            case 5
                ...
                % followed by a MSE minimization (Philippe)
                [u, v] = mapping_LI_classic(data_time,cartesian_grid,params);
            case 6
                % Local Interpolation through the Weighted Least Squares Minimization of all the radial currents at once (Philippe,Lucio)
