...
function [vr,threshold] = outliers_suppression_t(time, xr, yr, vr, meth, help_thresh)
...
params.time           = 1;      % outliers based on the statistics of the time-domain gradient of current values ("filtre_t")
                                                        %       1: on, 0: off
params.time_method    = meth;      % method for the evaluation of the maximum allowed current velocity time-domain gradient; 1: global; 2:par pixel; 3:fixe
params.time_percent   = 3;      % percentage of the maximum pdf of the current velocity time-domain gradient (method == 1)
params.time_tresh_grad_std  = 2; % JM; 0=no use; else value of factor for gradient standard deviation use as treshold
params.time_max_grad = 0.2;
%
